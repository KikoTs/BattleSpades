...
void AceMap::get_random_point(int *x, int *y, int *z, int x1, int y1, int x2, int y2) {
    ...
    // Start with a 300 block radius from center, decrease by 50 on each iteration
    int max_radius = 300;
    int radius_step = 50;
    
    bool valid_point_found = false;
    int rx = center_x, ry = center_y, rz = 0;
    
    // Try different radii, starting from max_radius and decreasing
    for (int radius = max_radius; radius >= radius_step; radius -= radius_step) {
        // Try several random points at this radius
        for (int attempts = 0; attempts < 15; attempts++) {
            // Generate a random angle
            double angle = this->eng() / (double)this->eng.max() * 2.0 * 3.14159265359;
            
            // Calculate point on the circle
            int offset_x = static_cast<int>(cos(angle) * radius);
            int offset_y = static_cast<int>(sin(angle) * radius);
            
            // Calculate actual map coordinates
            int temp_x = center_x + offset_x;
            int temp_y = center_y + offset_y;
            
            // Ensure we're within map boundaries
            if (!is_valid_pos(temp_x, temp_y, 0)) {
                continue;
            }
            
            // Get height at this position
            int temp_z = this->get_z(temp_x, temp_y);
            
            // Check if this is a valid spawn point
            if (temp_z < MAP_Z - 2 && 
                this->get_solid(temp_x, temp_y, temp_z) && 
                !is_water(temp_x, temp_y, temp_z) && 
                !this->get_solid(temp_x, temp_y, temp_z + 1) && 
                !this->get_solid(temp_x, temp_y, temp_z + 2)) {
        ...
        // If we found a valid point at this radius, we're done
        if (valid_point_found) {
            break;
        }
    }
    
    // Fallback: If no valid point was found, use center or surrounding area
    if (!valid_point_found) {
        rx = center_x;
        ry = center_y;
        rz = this->get_z(center_x, center_y);
        
        // If center isn't valid, search a small area around it
        if (rz >= MAP_Z - 2 || !this->get_solid(rx, ry, rz)) {
            for (int search_x = center_x - 25; search_x < center_x + 25; search_x++) {
                for (int search_y = center_y - 25; search_y < center_y + 25; search_y++) {
                    if (!is_valid_pos(search_x, search_y, 0)) continue;
                    
                    int search_z = this->get_z(search_x, search_y);
                    if (search_z < MAP_Z - 2 && this->get_solid(search_x, search_y, search_z)) {
                        rx = search_x;
                        ry = search_y;
                        rz = search_z;
                        valid_point_found = true;
...
bool AceMap::is_water(int x, int y, int z) {
    // Use a more reliable method to detect water - check if the block color
    // has a strong blue component and low red/green components
    uint32_t color = this->get_color(x, y, z);
    
    // Extract RGB components (assuming RGBA format 0xRRGGBBAA)
    uint8_t red = (color >> 24) & 0xFF;
    uint8_t green = (color >> 16) & 0xFF;
    uint8_t blue = (color >> 8) & 0xFF;
    
    // Water typically has high blue value, low red/green
    return (blue > 180 && red < 100 && green < 150) || 
           (blue > red + green);
}
